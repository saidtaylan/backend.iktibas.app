...
const UPLOAD_URL = `https://generativelanguage.googleapis.com/v1beta/files`;
 ...
 */
async function uploadFileToGemini(base64Data: string, mimeType: string, apiKey: string): Promise<any> {
 ...
 * @param apiKey Gemini API anahtarı
 */
async function askGeminiWithFile(fileName: string, prompt: string, mimeType: string, apiKey: string): Promise<string> {
...
serve(async (req) => {
  ...
  async function upsertQuote(status: string) {
    try {
  ...
  try {
    ...
    {
      if (geminiKeys.length === 1) return { key: geminiKeys[0], index: 0 };
      ...
      const { data, error } = await adminClient.rpc('gemini_rr_next', { total: geminiKeys.length });
    ...
    try {
      ...
      try {
        ocrText = await Promise.race([geminiPromise, timeoutPromise]);
